...
class KMeansML(val k: Int, val maxIterations: Int, val input: String, val centerInput: String, val output: String, master: String) extends Algorithm(master) {
  ...
  def run() = {
    ...
    val data = sc.textFile(input).map { line =>
      val l = line.split(',')
      eu.stratosphere.emma.experiments.spark.kmeans.ml.Vectors.dense(l.drop(1).map(_.toDouble))
    }
    ...
    val model = eu.stratosphere.emma.experiments.spark.kmeans.ml.KMeans.train(data, k, maxIterations, 1, centers)
