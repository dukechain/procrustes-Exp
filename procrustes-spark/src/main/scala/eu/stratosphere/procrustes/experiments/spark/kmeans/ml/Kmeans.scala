...
package eu.stratosphere.procrustes.experiments.spark.kmeans.ml
 ...
 */
class KMeans private ( private var centersInput: Array[Vector],
                       private var k: Int,
   ...
   */
  def this() = this(null ,3, 5, 1, KMeans.K_MEANS_PARALLEL, 5, 1e-4)
  ...
  def setCenters(centers: Array[Vector]): this.type = {
    this.centersInput = centers
    this
  }
  ...
  private def readCentersFromHDFS(): Array[Array[BreezeVectorWithNorm]] = {

    val norms = centersInput.map(v => breezeNorm(v.toBreeze, 2.0))
    val centers = centersInput.map(_.toBreeze).zip(norms).map { case (v, norm) =>
      new BreezeVectorWithNorm(v, norm)
    }.toArray
    Array(centers)
//
...
//    }.toArray)
  ...
  private def runBreeze(data: RDD[BreezeVectorWithNorm]): KMeansModel = {
    ...
    val centers = readCentersFromHDFS()
...
//        if (!changed) {
//          active(run) = false
//          logError("Run " + run + " finished in " + (iteration + 1) + " iterations")
//        }
...
object KMeans {
  ...
  def train(
             ...
             maxIterations: Int,
             runs: Int,
             centers: Array[Vector]): KMeansModel = {
    new KMeans().setK(k)
