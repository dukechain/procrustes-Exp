 ...
 * <li>Links are represented as pairs of page IDs which are separated by space characters. Links are
 * separated by new-line characters.<br>
 * For example <code>"1 2\n2 12\n1 12\n42 63\n"</code> gives four (directed) links (1)->(2),
 * (2)->(12), (1)->(12), and (42)->(63).<br>
 * For this simple implementation it is required that each page has at least one incoming and one
 * outgoing link (a page can point to itself).
 * </ul>
 * 
 * <p>
 * Usage:
 ...
 * <br>
 * If no parameters are provided, the program is run with default data from {@link PageRankData} and
 * 10 iterations.
 * 
 * <p>
...
public class PageRankLateCpt {
    ...
    public static void main(String[] args) throws Exception {
        ...
        .join(adjacencyListInput, JoinHint.REPARTITION_SORT_MERGE)
                                                          .where(0)
                                                          .equalTo(0)
        ...
        FileOutputFormat<Tuple2<Long, Double>> outputFormat =
        ...
        DataSet<Tuple2<Long, Double>> finalPageRanks =
                iteration.closeWith(newRanks.map(new MapFunction<Tuple2<Long, Double>, Tuple2<Long, Double>>() {
    ...
    private static DataSet<Long> getPagesDataSet(ExecutionEnvironment env) {
        // return
        // env.readCsvFile(pagesInputPath).fieldDelimiter(" ").lineDelimiter("\n").types(Long.class).map(new
        ...
        return env.readCsvFile(pagesInputPath)
                  ...
                  .types(Long.class)
                  .union(env.readCsvFile(pagesInputPath)
                            .fieldDelimiter("\t")
                            .ignoreComments("#")
                            .ignoreInvalidLines()
    ...
    private static DataSet<Tuple2<Long, Long>> getLinksDataSet(ExecutionEnvironment env) {
        // return
        // env.readCsvFile(linksInputPath).fieldDelimiter(" ").lineDelimiter("\n").types(Long.class,
        // Long.class);
        return env.readCsvFile(linksInputPath).fieldDelimiter("\t").ignoreComments("#").ignoreInvalidLines().types(Long.class, Long.class);
    }
}
