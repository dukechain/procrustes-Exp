...
public class KMeansTuple {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataSet<Tuple1<Centroid>> centroids = getCentroidDataSet(env);

        // set number of bulk iterations for KMeans algorithm
        IterativeDataSet<Tuple1<Centroid>> loop = centroids.iterate(numIterations);

        if (checkpointInterval > 0) {
            loop.setCheckpointInterval(checkpointInterval);
        }

        DataSet<Tuple1<Centroid>> newCentroids = points
        ...
        .map(new SelectNearestCenter())
                                                       ...
                                                       // compute new centroids from point counts
                                                       // and
                                                       // coordinate sums
                                                       .map(new CentroidAverager());

        // feed new centroids back into next iteration
        DataSet<Tuple1<Centroid>> finalCentroids = loop.closeWith(newCentroids);
    ...
    public static final class SelectNearestCenter extends RichMapFunction<Point, Tuple2<Integer, Point>> {
        ...
        private Collection<Tuple1<Centroid>> centroids;
        ...
        public Tuple2<Integer, Point> map(Point p) throws Exception {
            ...
            for (Tuple1<Centroid> tuple_centroid : centroids) {

                Centroid centroid = tuple_centroid.f0;
    ...
    public static final class CentroidAverager implements MapFunction<Tuple3<Integer, Point, Long>, Tuple1<Centroid>> {

        @Override
        public Tuple1<Centroid> map(Tuple3<Integer, Point, Long> value) {
            return new Tuple1<Centroid>(new Centroid(value.f0, value.f1.div(value.f2)));
        }
    }
    ...
    private static DataSet<Tuple1<Centroid>> getCentroidDataSet(ExecutionEnvironment env) {
